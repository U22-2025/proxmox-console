...
- **識別子のみ保持** — VMのスペック・状態はProxmox APIから取得し、DBとの乖離を防ぐ
- **SQLite** — 外部依存なし、単一バイナリで動作
- **ネットワーク情報はDB管理** — VLAN IDとサブネットの割り当て
...
| proxmox_vm_id | INTEGER | NOT NULL | Proxmoxが割り当てたVM ID |
| node_name | TEXT | NOT NULL | 稼働ノード名 |
| tf_workdir | TEXT | NOT NULL | Terraform実行ディレクトリ（destroy用） |
| status | TEXT | NOT NULL, DEFAULT 'creating' | `creating` / `updating` / `active` / `error` |
| created_at | DATETIME | NOT NULL | 作成日時 |
...
| `active` | 作業完了、Proxmoxで稼働可能 | → `updating` / `error` |
